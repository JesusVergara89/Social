    ...
    const myTimes = (creatorID, receptorID, userNameR, userNameS) => {
        if (timer) {
            const EnterTestExistance = timer.find(obj => {
                return ((obj.data[0].creatorID === creatorID && obj.data[0].receptorID === receptorID) || (obj.data[0].creatorID === receptorID && obj.data[0].receptorID === creatorID))
            });
            //console.log(EnterTestExistance)
            if (EnterTestExistance) {
